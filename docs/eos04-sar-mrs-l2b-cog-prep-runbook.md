...
`scripts/prepare_eos04_sar_mrs_l2b_cogs.py` -> `worker.py ingest-manifest`.
...
```bash
python scripts/prepare_eos04_sar_mrs_l2b_cogs.py --zip-path <raw>/<product>.zip
```
...
- API: `GET /api/sources` lists `eos-04-sar-mrs-l2b` as an active SAR source;
  `GET /api/sources/eos-04-sar-mrs-l2b/dates` returns the ingested date with
...
- The source is already registered and activated (display-only) in
