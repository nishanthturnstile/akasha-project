"""Prepare Akasha NISAR S-SAR Beta GCOV backscatter COGs.

NISAR GCOV (Geocoded Covariance) products from ISRO Bhoonidhi are already
geocoded, so — like EOS-04 SAR-MRS L2B and unlike Sentinel-1 GRD — no ESA SNAP
step is required. GCOV products are typically delivered as **HDF5** (``.h5``)
containing per-polarization covariance grids (the diagonal terms ``HHHH``,
``HVHV``, ``VHVH``, ``VVVV`` are backscatter power in linear units); some
distributions provide geocoded GeoTIFFs instead. This script reads the diagonal
backscatter term(s), converts to a dB scale, and writes a deterministic
``backscatter.tif`` COG plus a ``prepare_manifest.json`` matching the SAR
ingestion contract consumed by ``akasha_ingest.scene.SceneIdentity`` and
``akasha_ingest.catalog`` / ``akasha_ingest.storage``.

Outputs are written under:

    data/seed/rasters/nisar-ssar-beta-gcov/{acquisitionDate}/{relativeOrbitOrUnknown}/{sceneComponent}/backscatter.tif
    data/seed/rasters/nisar-ssar-beta-gcov/{acquisitionDate}/{relativeOrbitOrUnknown}/{sceneComponent}/prepare_manifest.json

Step 0 (format validation): inspect a real sample with
``gdalinfo NISAR_..._GCOV_....h5`` to list the HDF5 subdatasets (or
``gdalinfo`` a GeoTIFF), confirm which covariance-diagonal layers carry the
backscatter, and confirm whether values are linear power or already dB. Set
``--polarizations``, ``--input-scale``, and (if needed) ``--band-path`` to match.
"""
...
BHOONIDHI_COLLECTION = "NISAR_SSAR-Beta_GCOV"
...
RUNBOOK_PATH = "docs/nisar-ssar-beta-gcov-cog-prep-runbook.md"
PROCESSING_VERSION = "akasha-nisar-ssar-beta-gcov-v1"
DEFAULT_NODATA = -9999.0
DB_EPSILON = 1e-8
COG_BLOCKSIZE = 512
DEFAULT_VV_RESCALE = "-25,5"

KNOWN_POLARIZATIONS = ("HH", "HV", "VH", "VV")
DEFAULT_POLARIZATION = "HH"
POL_DISPLAY_ORDER = {pol: index for index, pol in enumerate(KNOWN_POLARIZATIONS)}
# GCOV covariance diagonal terms (linear backscatter power) -> polarization.
COVARIANCE_DIAGONAL_TERMS = {"HHHH": "HH", "HVHV": "HV", "VHVH": "VH", "VVVV": "VV"}
...
def selected_product_from_manifest_entry(
    ...
    return SelectedProduct(
        ...
        platform=_entry_value(entry, "platform") or "nisar",
...
def latest_source_path(raw_dir: Path) -> Path:
    ...
    if not candidates:
        raise SystemExit(f"No NISAR product ZIP/HDF5 found under {raw_dir}. Pass --zip-path.")
...
def extract_product(source_path: Path, extract_dir: Path, *, overwrite: bool) -> Path:
    """Return a directory (or single file's parent) containing the product."""
    if source_path.is_dir():
        return source_path
    if not source_path.exists():
        raise SystemExit(f"NISAR product not found: {source_path}. See {RUNBOOK_PATH}.")
    if source_path.suffix.lower() != ".zip":
        # A bare .h5 / GeoTIFF was provided; use its parent as the product dir.
...
def _polarization_from_token(name: str) -> str | None:
    upper = name.upper()
    # GCOV covariance diagonal term (e.g. .../frequencyA/HHHH) wins.
    for term, pol in COVARIANCE_DIAGONAL_TERMS.items():
        if re.search(rf"(?:^|[^A-Z])({term})(?:[^A-Z]|$)", upper):
            return pol
...
def _hdf5_backscatter_subdatasets(deps: dict[str, Any], h5_path: Path) -> list[tuple[str, str]]:
    """Return (polarization, rasterio-openable subdataset string) for GCOV terms."""
    rasterio = deps["rasterio"]
    with rasterio.open(h5_path) as dataset:
        subdatasets = list(dataset.subdatasets or [])
    found: dict[str, str] = {}
    for sub in subdatasets:
        leaf = sub.rstrip("/").rsplit("/", 1)[-1].rsplit(":", 1)[-1]
        pol = COVARIANCE_DIAGONAL_TERMS.get(leaf.upper())
        if pol and pol not in found:
            found[pol] = sub
    return [(pol, found[pol]) for pol in sort_polarizations(list(found))]
...
def find_backscatter_bands(
    ...
    product_dir: Path,
    polarizations: list[str],
    explicit_band: str | None,
) -> list[tuple[str, str]]:
    """Map polarization -> rasterio-openable target (file path or HDF5 subdataset)."""
    ...
    h5_files = sorted(
        path
        for pattern in ("*.h5", "*.H5")
        for path in product_dir.rglob(pattern)
        if path.is_file()
    )
    for h5_path in h5_files:
        bands = _hdf5_backscatter_subdatasets(deps, h5_path)
        if bands:
            mapping = dict(bands)
            wanted = [pol for pol in polarizations if pol in mapping] or [pol for pol, _ in bands]
            return [(pol, mapping[pol]) for pol in wanted]
    ...
    if not tifs:
        raise SystemExit(
            f"No NISAR GCOV HDF5 subdataset or GeoTIFF found under {product_dir}. "
            f"Pass --band-path. See {RUNBOOK_PATH}."
        )
    by_pol: dict[str, str] = {}
    for path in tifs:
        pol = _polarization_from_token(path.name)
    ...
    return [(pols[index], str(path)) for index, path in enumerate(tifs[: len(pols)])]
...
def detect_input_scale(np: Any, sample: Any, requested: str) -> str:
    ...
    GCOV covariance diagonal terms are linear backscatter power, so 'auto'
    defaults to 'linear' unless the data is clearly already in dB.
    """
    if requested != "auto":
        return requested
...
def write_backscatter_db_intermediate(
    *,
    ...
    reference_target = bands[0][1]
    resolved_scale = input_scale
    with rasterio.open(reference_target) as ref:
        ...
        with rasterio.open(output_path, "w", **profile) as dst:
            for dst_index, (pol, target) in enumerate(bands, start=1):
                with rasterio.open(target) as src:
                    if (src.width, src.height) != (ref.width, ref.height):
                        raise SystemExit(
                            f"Band {target} grid {(src.width, src.height)} does not match "
                ...
                if dst_index == 1:
                    ...
                    print(f"input scale for {target}: {resolved_scale}")
                ...
                dst.update_tags(
                    dst_index,
                    name=f"{pol}_dB",
                    source_band=str(target),
                    input_scale=resolved_scale,
                    formula={
                        "db": "passthrough (already dB)",
                        "linear": f"10*log10(max(gamma0,{DB_EPSILON}))",
...
def write_manifest(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "processing": {
            "engine": "rasterio + rio-cogeo (no SNAP; GCOV is pre-geocoded)",
            "steps": [
                "Read NISAR GCOV covariance-diagonal backscatter layer(s)",
...
def prepare_one(
    *, product: SelectedProduct, args: argparse.Namespace, deps: dict[str, Any]
    ...
    explicit_band = args.band_path or None
    bands = find_backscatter_bands(deps, product_dir, product.polarizations, explicit_band)
    print("bands: " + ", ".join(f"{pol}->{target}" for pol, target in bands))
    ...
    write_manifest(
        ...
        input_scale=resolved_scale,
        band_sources=[str(target) for _, target in bands],
        args=args,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--zip-path", help="Path to a single NISAR GCOV product ZIP/HDF5/dir")
    ...
    parser.add_argument("--platform", help="Platform, default nisar")
    ...
    parser.add_argument("--instrument-mode", help="SAR instrument mode, default GCOV")
    parser.add_argument("--product-type", help=f"Product type, default {BHOONIDHI_COLLECTION}")
    parser.add_argument(
        "--polarizations",
        help="Comma-separated polarizations (e.g. HH,HV). Inferred from GCOV terms when omitted.",
    )
    parser.add_argument(
        "--band-path",
        help="Explicit backscatter target (GeoTIFF path or HDF5 subdataset string)",
    )
    parser.add_argument(
        "--input-scale",
        choices=("auto", "linear", "amplitude", "db"),
        default="auto",
        help="Calibration scale of the source pixels; 'auto' assumes linear GCOV power.",
    )
    parser.add_argument(
    ...
    print("NISAR S-SAR Beta GCOV COG preparation complete")
    return 0
