...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-031 | In `services/ingestion/akasha_ingest/pipeline_registry.py`, verify `resourcesat-2a-awifs-boa` has `provider="bhoonidhi"`, `collection_id="ResourceSat-2A_AWIFS_BOA"`, `prepare_script="prepare_resourcesat_liss3_boa_cogs.py"`, `supports_search=True`, `supports_download=True`, `supports_composite=True`, `mvp_enabled=True`, `default_min_coverage_percent=95.0`, and `output_profile="resourcesat-awifs-boa"`. | ✅ | 2026-06-23 |
| TASK-032 | In `services/ingestion/akasha_ingest/bhoonidhi.py`, verify `SOURCE_COLLECTIONS["resourcesat-2a-awifs-boa"] == "ResourceSat-2A_AWIFS_BOA"`. Add a test if this mapping is not already asserted. | ✅ | 2026-06-23 |
| TASK-033 | In `scripts/prepare_resourcesat_liss3_boa_cogs.py`, verify `SOURCE_PROFILES["resourcesat-2a-awifs-boa"]` has `label="AWiFS"`, `resolution_meters=56`, `analytic_bands=LISS3_ANALYTIC_BANDS`, `reflectance_scale=0.0001`, `reflectance_offset=0.0`, `mask_builder="4band"`, and an AWiFS-specific `mask_method` string. | ✅ | 2026-06-23 |
| TASK-034 | Add `tests/test_prepare_resourcesat_awifs_boa_cogs.py`. Build synthetic four-band AWiFS inputs named `BAND2.tif`, `BAND3.tif`, `BAND4.tif`, `BAND5.tif`; run the prepare script with `--source resourcesat-2a-awifs-boa`; assert the output manifest has 4 bands, `resolution_meters=56`, source ID `resourcesat-2a-awifs-boa`, collection `ResourceSat-2A_AWIFS_BOA`, and mask classes `{0,1,2,3,4}`. | ✅ | 2026-06-23 |
| TASK-035 | Run `python -m pytest tests/test_prepare_resourcesat_awifs_boa_cogs.py -q`. Expected result: AWiFS synthetic prepare test passes. If raster dependencies are unavailable on the host, run the test inside the ingestion container and record the command in the plan execution notes. | ✅ | 2026-06-23 |
| TASK-036 | In `services/ingestion/akasha_ingest/composite.py`, verify `SOURCE_PROFILES["resourcesat-2a-awifs-boa"]` uses `resolution=56.0`, `analytic_band_order=["BAND2","BAND3","BAND4","BAND5"]`, and the four-band ResourceSat role mapping. Add a composite unit test that AWiFS produces a 56 m composite grid and preserves 4 analytic bands. | ✅ | 2026-06-23 |
...
| TASK-038 | Add `tests/test_bhoonidhi_ingestion.py::test_catalog_emits_awifs_resourcesat_item`. Use an AWiFS composite manifest and assert collection `resourcesat-2a-awifs-boa`, instruments `["awifs"]`, 4 analytic EO bands, 4 raster bands, asset hrefs under `s3://akasha-cogs/resourcesat-2a-awifs-boa/composite/bangalore-60km/{date}/`, `akasha:band_role_mapping` includes SWIR1, and `gsd == 56`. | ✅ | 2026-06-23 |

Phase 4 validation notes (2026-06-23):

- TASK-031 evidence: `tests/test_pipeline_registry.py::test_awifs_pipeline_source_declares_regional_production_profile` verifies AWiFS provider `bhoonidhi`, collection `ResourceSat-2A_AWIFS_BOA`, shared ResourceSat prepare script, search/download/composite support, MVP enablement, `default_min_coverage_percent=95.0`, and `output_profile="resourcesat-awifs-boa"`.
- TASK-032 evidence: `tests/test_bhoonidhi_ingestion.py::test_source_collection_supports_awifs_phase5_source` verifies `bhoonidhi.source_collection("resourcesat-2a-awifs-boa") == "ResourceSat-2A_AWIFS_BOA"`.
- TASK-033/TASK-034/TASK-035 evidence: `tests/test_prepare_resourcesat_awifs_boa_cogs.py` builds a synthetic AWiFS product directory with `BAND2.tif`, `BAND3.tif`, `BAND4.tif`, `BAND5.tif`, and `BAND_META.txt`, runs `scripts.prepare_resourcesat_liss3_boa_cogs.main()` with `--source resourcesat-2a-awifs-boa`, and verifies the manifest source ID, collection, 4-band analytic order, SWIR1 role mapping, 56 m resolution, 1-band mask, mask classes `{0,1,2,3,4}`, and AWiFS-specific provisional mask method. The red/green cycle exposed and fixed a cross-platform `find_band_asset()` bug where `.tif`/`.TIF` glob patterns double-counted the same file on Windows; the function now de-duplicates resolved paths while preserving uppercase-product support.
- TASK-036 evidence: `tests/test_resourcesat_composite.py::test_build_resource_sat_composite_defaults_awifs_resolution` now verifies the AWiFS composite path defaults to a 56 m grid and preserves 4 analytic bands through the composite writer.
- TASK-037 evidence: `services/ingestion/akasha_ingest/catalog.py` has AWiFS source metadata `instrument="awifs"`, `label="AWiFS"`, and `default_gsd=56`. AWiFS-specific EO wavelength metadata has not yet been validated from NRSC product metadata, so STAC generation intentionally keeps the shared ResourceSat broad-band EO metadata alias. `apps/api/app/raster/catalog_resolver.py` now documents that assumption in AWiFS source `limitations` while keeping AWiFS `availabilityStatus="gated"`.
- TASK-038 evidence: `tests/test_bhoonidhi_ingestion.py::test_catalog_emits_awifs_resourcesat_item` verifies manifest-derived AWiFS composite STAC item generation with collection `resourcesat-2a-awifs-boa`, instruments `["awifs"]`, 4 EO/raster analytic bands, 1 mask raster band, `SWIR1 -> BAND5` role mapping, `gsd == 56`, and S3 asset hrefs under `s3://akasha-cogs/resourcesat-2a-awifs-boa/composite/bangalore-60km/2026-03-19/`.
- Verification evidence: focused Phase 4 tests completed with `5 passed in 1.32s`. Final verification ran `ruff check scripts/prepare_resourcesat_liss3_boa_cogs.py tests/test_prepare_resourcesat_awifs_boa_cogs.py tests/test_pipeline_registry.py tests/test_bhoonidhi_ingestion.py tests/test_resourcesat_composite.py apps/api/tests/test_product_sources.py apps/api/app/raster/catalog_resolver.py` with `All checks passed!`; broader ResourceSat Phase 4 regression suite `python -m pytest tests/test_pipeline_registry.py tests/test_bhoonidhi_ingestion.py tests/test_prepare_resourcesat_liss3_boa_cogs.py tests/test_prepare_resourcesat_awifs_boa_cogs.py tests/test_resourcesat_composite.py -q` completed with `94 passed in 4.49s`; BFF source metadata check `cd apps/api && python -m pytest tests/test_product_sources.py tests/test_slice2.py::test_sources_endpoint_contract -q` completed with `3 passed in 6.38s`.
...
| TASK-043 | Add or update BFF source tests asserting `/api/sources` returns AWiFS as `availabilityStatus="gated"`, `analysisLevel="regional"`, `resolutionMeters=56`, and the exact gated reason `No validated AWiFS BOA composite has been ingested.` before TASK-049 passes. In TASK-053, update the same test to assert `availabilityStatus="active"` and `gatedReason is None` after AWiFS activation. | ✅ | 2026-06-23 |
| TASK-044 | In `apps/frontend/src/components/layers/SourceCard.tsx` and `SourceMetadata.tsx`, verify gated regional sources render the gated badge, regional analysis label, limitations, and gated reason. Add a Vitest test if AWiFS-specific regional copy is not covered by existing EOS-06/Cartosat gated tests. | ✅ | 2026-06-23 |
| TASK-045 | Run frontend source card tests from `apps/frontend`: `yarn test SourceCard SourceMetadata`. Expected result: gated AWiFS/regional source UI behavior passes. If the repo uses `corepack yarn`, run `corepack yarn test SourceCard SourceMetadata`. | ✅ | 2026-06-23 |

Phase 5 validation notes (2026-06-23):

- TASK-039/TASK-040/TASK-043 evidence: `apps/api/app/raster/catalog_resolver.py` keeps AWiFS gated with `gatedReason="No validated AWiFS BOA composite has been ingested."`, `analysisLevel="regional"`, `resolutionMeters=56`, supported indices `NDVI/MSAVI/NDMI/NDWI_GREEN_NIR`, `displayModes=["FCC","NDVI","MSAVI","NDMI","NDWI_GREEN_NIR"]`, `mapDisplayModes=["NDVI","MSAVI","NDMI","NDWI_GREEN_NIR"]`, `defaultDisplayMode="FCC"`, `defaultMapDisplayMode="NDVI"`, and limitations that communicate coarse 56 m regional use, provisional threshold mask, pending AWiFS EO wavelength validation, and non-replacement of LISS-3/LISS-4 field monitoring. TDD red evidence: the focused BFF tests failed while AWiFS still advertised only `displayModes=["FCC"]`; after the metadata update, `cd apps/api && python -m pytest tests/test_product_sources.py::test_awifs_source_payload_stays_gated_with_regional_limitations tests/test_slice2.py::test_sources_endpoint_contract tests/test_slice2.py::test_awifs_seed_collection_contract_is_regional_and_gated -q` completed with `3 passed in 3.13s`.
- TASK-041 evidence: `data/seed/stac/resourcesat-2a-awifs-boa-collection.json` now mirrors the BFF registry for AWiFS source ID, Bhoonidhi collection ID, regional analysis level, gated status/reason, FCC default display, index display modes, supported indices, 4-band EO/raster metadata, reflectance scale/offset, mask asset, and mask classification classes. `apps/api/tests/test_slice2.py::test_awifs_seed_collection_contract_is_regional_and_gated` locks this STAC collection contract.
- TASK-042 evidence: `tests/test_bhoonidhi_ingestion.py::test_awifs_sample_items_are_not_loaded_as_production_seed_items` defines an in-test placeholder AWiFS sample item and asserts `config.item_files("resourcesat-2a-awifs-boa") == []`; the focused test completed with `1 passed in 0.56s`.
- TASK-044/TASK-045 evidence: `apps/frontend/src/components/layers/SourceCard.test.tsx` and `SourceMetadata.test.tsx` now cover AWiFS gated regional UI behavior: `Regional gated` metadata label, gated reason, regional/coarse limitations, and available AWiFS index display-mode controls. `cd apps/frontend && corepack yarn test SourceCard SourceMetadata` completed with `2 passed` test files and `6 passed` tests in `13.00s`.
- Verification evidence: final Phase 5 scoped lint/test command completed with `All checks passed!`, BFF source/STAC tests `4 passed in 2.64s`, AWiFS seed-item guard `1 passed in 0.56s`, and frontend SourceCard/SourceMetadata tests `6 passed`.
...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-046 | On `akasha-staging`, run `python worker.py bhoonidhi-search --source resourcesat-2a-awifs-boa --aoi bangalore-60km --lookback-days 120 --limit 100`. Acceptance: command exits 0, writes a coverage manifest, and selected candidates come from `ResourceSat-2A_AWIFS_BOA`. | ✅ | 2026-06-23 |
...
| TASK-048 | Inspect the downloaded AWiFS product with `gdalinfo` or rasterio. Acceptance: product has `BAND2`, `BAND3`, `BAND4`, `BAND5`; dtype is uint16 or documented equivalent; CRS and dimensions are recorded; scale/offset/background are recorded; no native quality mask is required for pipeline success. Store findings in `test_reports/awifs-validation-YYYY-MM-DD.md`. | ✅ | 2026-06-23 |
| TASK-049 | On `akasha-staging`, run `python worker.py bhoonidhi-sync --source resourcesat-2a-awifs-boa --aoi bangalore-60km --max-downloads 3 --min-coverage-percent 95 --method upsert`. Acceptance: search, download, prepare, composite, upload, STAC registration, and post-ingest `verify-composite` complete with exit code 0. If the first run has insufficient cloud-free coverage, keep AWiFS gated and run another window; do not reduce the threshold below 95 without documenting why the regional source still meets launch needs. | ⛔ Blocked | 2026-06-23 |
| TASK-050 | Run `GET /api/sources/resourcesat-2a-awifs-boa/dates` against the deployed BFF. Acceptance: after TASK-049 passes, the response contains at least one date with `tileAvailable=true`, `coveragePercent >= 95`, and `metricsProvisional=true`. | ⛔ Blocked | 2026-06-23 |
| TASK-051 | Run tile smoke for AWiFS FCC through the gateway using a same-origin route: `/api/tiles/resourcesat-2a-awifs-boa/{acquisitionDate}/FCC/{z}/{x}/{y}.png`. Acceptance: HTTP 200 for a tile inside AOI coverage and no direct MinIO/TiTiler URL exposed to the browser. | ⛔ Blocked | 2026-06-23 |
| TASK-052 | Run a statistics smoke test for a large field or AOI-sized polygon using `sourceId=resourcesat-2a-awifs-boa` and `indexType=NDVI`. Acceptance: response includes valid statistics, source ID AWiFS, regional limitations remain visible in `/api/sources`, and no NDRE/RECI support is advertised. | ⛔ Blocked | 2026-06-23 |
| TASK-053 | Flip AWiFS to active only after TASK-049, TASK-050, TASK-051, and TASK-052 pass. In `apps/api/app/raster/catalog_resolver.py`, set `availabilityStatus="active"`, `gatedReason=None`, keep `analysisLevel="regional"`, keep limitations, and keep `metricsProvisional=true`. Update `docs/data-ingestion-and-satellite-rules.md` and `docs/reference/satellite-ingestion-onboarding-matrix.md` with the validation date and acceptance evidence. | ⛔ Blocked | 2026-06-23 |

Phase 6 validation notes (2026-06-23):

- Safety evidence: all AWiFS Bhoonidhi/search/download/prepare/composite work was run via the safe wrapper path `python scripts/staging_ingestion_job.py trigger --host akasha-staging ...`. No direct staging `docker run`, `docker compose run`, or direct `worker.py` ingestion/composite/verification commands were used.
- TASK-046 evidence: safe-wrapper dry-run job `ingest-20260623T175748Z-a810615f` completed with state `succeeded`. Log output included `found 9 Bhoonidhi item(s)`, `selected 9 candidate(s)`, `new products 9`, `sync window: 2026-02-24..2026-06-23`, and a coverage manifest at `/srv/akasha/data/work/bhoonidhi/resourcesat-2a-awifs-boa/bangalore-60km/coverage_manifest.new.json`; dry-run stopped before download/prepare/composite/ingest.
- TASK-047/TASK-048 evidence: capped safe-wrapper job `ingest-20260623T175830Z-697347bf` downloaded `RAW20MAR2026048167010400065PSANSTLCSRHTDC.zip` (`300606167` bytes) and prepared a scene manifest under `/app/data/seed/rasters/resourcesat-2a-awifs-boa/scene/2026-03-20/.../prepare_manifest.json`. Retained AWiFS ZIPs inspected on staging contain `BAND2.tif`, `BAND3.tif`, `BAND4.tif`, `BAND5.tif`, `BAND_META.txt`, and product `.meta`. Metadata reports `SENSOR=AW`, `NOOFBANDS=4`, `BANDNUMBERS=2345`, `BYTESPERPIXEL=2`, `BITSPERPIXEL=12`, `OUTPUTRESOLUTIONALONG=56.00`, `OUTPUTRESOLUTIONACROSS=56.00`, `IMAGEFORMAT=GEOTIFF`, `PROCESSINGLEVEL=AtmosphericallyCorrectedProduct`, and `MAPPROJECTION=LCC`. Findings are recorded in `test_reports/awifs-validation-2026-06-23.md`.
- TASK-049 blocker evidence: the first capped job failed validation with `coverage 0.0% below threshold 95.0%`. Read-only mask inspection showed the prepared scene mask had `79.726034%` non-zero coverage while the composite mask had `0.0%`, indicating product structure and scene mask generation succeeded but the first date-ordered candidate was edge-only for the launch AOI. A second bounded safe-wrapper retry `ingest-20260623T180332Z-de5f08a9` with `max-downloads=3 --overwrite` downloaded/prepared three additional AWiFS products and rebuilt the composite, but validation still failed with `coverage 62.9839% below threshold 95.0%`. Per the plan, AWiFS remains gated; do not lower the 95% regional acceptance threshold without a separate documented product decision.
- TASK-050/TASK-051/TASK-052/TASK-053 blocker: these tasks are intentionally not run because TASK-049 did not pass. AWiFS activation is blocked until a later bounded safe-wrapper run produces and verifies an AWiFS composite with `coveragePercent >= 95` and catalog registration succeeds.
